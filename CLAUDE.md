...
- **AI**: Google Gemini 2.0 Flash (REST API 직접 호출, `@google/generative-ai` 패키지 미사용)
...
2. Gemini 2.0 Flash로 식단 생성 (temperature: 식단 0.75 / Judge 0.1)
3. LLM-as-a-Judge로 결과 검증 → 실패 시 1회 재시도
4. 사용자당 하루 5회 제한 (`apiUsage` 컬렉션)

## 시크릿 / 환경 변수
- `GEMINI_API_KEY`: Firebase Secret Manager (functions에서 `defineSecret`으로 참조)
